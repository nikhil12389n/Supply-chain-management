...
export default function StatusOfReceived () {
    ...
    useEffect(()=>{
        let table=$("#statusofreceiveddgst").DataTable();
               ...
               for(let i=0;i<findata.length;i++){
                 if(findata[i].status!="pending" && findata[i].status!="accepted"){
                    try{
                        ...
                        if(d==true){
                            table.row.add([findata[i].hash,findata[i].from,findata[i].origin,Object.values(findata[i].products).join(","),Object.values(findata[i].quantities).join(","),findata[i].endtime,"declined by all adsts under divisions"]);
                        }
                        else{
    ...
    return (
        ...
        <h1>Status Of Received</h1>
            ...
            <table id="statusofreceiveddgst" className="table tablereqforsupply">
                <thead>
                    <tr>
                        <th>Hash</th>
                        <th>From</th>
